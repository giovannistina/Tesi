...
def analyze_power_law_fit(df_events):
    ...
    try:
        data['bin_X'] = pd.qcut(data['prev_X'], q=50, duplicates='drop')
    except ValueError:
        # Fallback se i dati sono troppo scarsi per 20 bin
        data['bin_X'] = pd.qcut(data['prev_X'], q=10, duplicates='drop')
    ...
    # Rimuoviamo bin vuoti (NaN)
    binned = binned.dropna()

    if len(binned) < 2:
        return 0.0, 0.0, 0.0, binned

    # Regressione Lineare in scala Log-Log
...
def main():
    print(f"--- Modulo C: Analisi Statistica Completa (FIX REPOST) ---")
    ...
    if os.path.exists(INPUT_EVENTS):
        print("📉 Caricamento Eventi per Fitting Theta...")
        
        # --- MODIFICA FONDAMENTALE: CARICHIAMO 'post_type' ---
        cols_to_load = ['prev_X', 'v_i', 'post_type']
        
        # Gestione chunk per file enormi (opzionale, ma sicuro)
        df_events = pd.read_csv(INPUT_EVENTS, usecols=cols_to_load, compression='gzip')
        print(f"   Righe Totali (Grezze): {len(df_events)}")
        
        # --- FILTRO: SOLO POST ORIGINALI ---
        df_clean = df_events[df_events['post_type'] == 'post'].copy()
        print(f"   ⬇️  Filtro Repost applicato.")
        print(f"   ✅ Post Originali analizzati: {len(df_clean)} (Scartati {len(df_events) - len(df_clean)} repost)")
        
        theta_est, intercept_est, r_val, binned_data = analyze_power_law_fit(df_clean)
        r_squared = r_val**2
    ...
    # 4. REPORT TESTUALE
    report = f"""
    === REPORT ANALISI (REPOST FILTRATI) ===
    
    1. DATI
       Utenti Totali: {len(df_users)}
       
    2. DISUGUAGLIANZA (Gini)
       - Popolarità (X): {gini_X:.4f}
       - Qualità (Beta): {gini_Beta:.4f}
       - Attività (Lambda): {gini_Lambda:.4f}
       
    3. PARAMETRI STIMATI (Solo su Post Originali)
       - Theta (Viralità): {theta_est:.4f}
       - R^2 del Fit: {r_squared:.4f}
       
    4. CORRELAZIONI
       - Lambda vs X: {corr_lambda_X:.4f}
       - Beta vs X: {corr_beta_X:.4f}
    """
    ...
    if binned_data is not None:
        ...
        plt.scatter(binned_data['prev_X'], binned_data['v_i'], color='black', label='Dati (Solo Post)')
        ...
        plt.xlabel(r"Popolarità $X(t^-)$")
        plt.ylabel(r"Successo $V_i$")
        plt.title(f"Viralità (No Repost): Theta={theta_est:.2f}")
        plt.legend()
        plt.savefig(os.path.join(OUTPUT_DIR_IMGS, "1b_fitting_theta.png"))
        plt.close()

    # Funzione helper plot
    def add_log_regression(x, y, ax, color_line='red'):
        mask = (x > 0) & (y > 0)
        x_clean = x[mask]; y_clean = y[mask]
        if len(x_clean) > 1:
            log_x = np.log(x_clean); log_y = np.log(y_clean)
            ...
            ax.plot(x_vals, y_vals, color=color_line, ls='--', lw=2, label=rf'Slope={slope:.2f}')
            return r_val
        return 0.0

    # FIG 3: Beta vs X
    plt.figure(figsize=(8, 8))
    plt.scatter(df_active['beta'], df_active['mean_X'], alpha=0.3, s=10, c='purple')
    add_log_regression(df_active['beta'], df_active['mean_X'], plt.gca(), color_line='orange')
    plt.xscale('log'); plt.yscale('log')
    plt.xlabel("Qualità Beta"); plt.ylabel("Popolarità X")
    plt.title("Qualità vs Fama")
    plt.savefig(os.path.join(OUTPUT_DIR_IMGS, "1c_scatter_beta_X.png"))
    plt.close()

    # FIG 4: Lambda vs X
    plt.figure(figsize=(8, 8))
    plt.scatter(df_active['lambda'], df_active['mean_X'], alpha=0.3, s=10, c='green')
    add_log_regression(df_active['lambda'], df_active['mean_X'], plt.gca(), color_line='blue')
    plt.xscale('log'); plt.yscale('log')
    plt.xlabel("Attività Lambda"); plt.ylabel("Popolarità X")
    plt.title("Attività vs Fama")
    plt.savefig(os.path.join(OUTPUT_DIR_IMGS, "1d_scatter_lambda_X.png"))
    plt.close()

    print(f"✅ Analisi completata (Repost esclusi).")
